...
public final class UserDaoTest {
	...
	public void testInsertUser() {
		...
		assertTrue(userDao.save(user).equals(user));
	}
	...
	public void showFields() throws Exception {
		...
		User user = new User();
		user.setNombre("Bruno");
		user.setApellido("Fdez");
		user.setEdad(2);
		user.setSoltero(true);
		user.setDni("4583434B");
		user.setTelefono("654852123");
		user.setEmail("abel@pruebas.com");
		...
		for (Field field : clazz.getDeclaredFields()) {
			System.out.println(field.getName() + ", value: " + field.getAnnotations());
		}

		for (Method method : clazz.getDeclaredMethods()) {

			if (method.getName().startsWith("get")) {
				System.out.println(method.getName().substring(3).toLowerCase() + " - " + method.invoke(user));
			}
